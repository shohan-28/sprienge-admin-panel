...
## v3.4 — Status lock (Courier Manager only) + Vercel fix

- **Status lock**: once a Steadfast parcel exists on an order
  (`courierStatus` is set), the status buttons on Order Details and the
  inline status dropdown on the Orders list both **lock for everyone
  except the Courier Manager admin** — a 🔒 icon and a short explanation
  show instead. Before a parcel exists, any admin can still change status
  freely, same as before.
- The backend webhook (see `backend-reference/` / the standalone backend
  project) now also **auto-syncs `order.status`** from terminal Steadfast
  outcomes (`delivered`, `cancelled`, `partial_delivered`, `returned`) —
  so the status shown is the courier's real-world truth, not just locked
  UI with stale data underneath.
- **Vercel reload fix**: added `vercel.json` with a catch-all rewrite to
  `index.html`. Without this, refreshing on any route other than `/` (e.g.
  `/orders/abc123`) 404s on Vercel, because it tries to find a real file
  at that path — this tells Vercel to always serve the SPA shell and let
  React Router handle the route client-side. Redeploy after adding it.

## v4.0 — Customer management, advanced inventory, reports, permissions, invoices, global search

This is a large batch — 13 of 15 requested feature categories (SMS/WhatsApp
notifications and full JWT/security hardening were explicitly excluded per
the user's request; see "Explicitly not built" below).

### Granular admin permissions (replaces the old single `canManageCourier` flag)
- `src/config/permissions.js` defines 3 roles as permission bundles:
  **Super Admin** (everything), **Order Manager** (orders + returns +
  export, no courier actions), **Courier Manager** (courier + settings,
  no delete/returns/finance).
- `src/config/admins.js` now builds each admin from a role — `canManageCourier`
  is kept as a mirror of `permissions.manageCourier` so old code paths
  still work unchanged.
- Gated actions: delete order (Orders list + Order Details), CSV export
  (Orders, Audit Log, every Reports tab), approve return/refund, view the
  Profit report tab and today's-profit stat.
- Add a 4th role by adding an entry to `ROLES` in `permissions.js` and
  wiring one more admin in `admins.js` — no other file needs touching.

### Customer Management (`/customers`)
- One profile per phone number, derived from existing orders (no separate
  backend collection needed): total orders, lifetime spend, delivery
  success rate, auto tag (VIP ≥5 orders or ≥৳15,000 spend / Regular /
  New), full order history inline, and a blacklist toggle with a reason.

### Inventory, upgraded
- Products now carry cost price, category, supplier, and an auto-generated
  barcode (rendered as a real scannable barcode image via barcodeapi.org —
  same "render via free public API" pattern as the existing QR codes).
- **Stock adjustments**: add/remove stock with a required reason, fully
  logged (adjustment history per product + the global Audit Log) — this
  is separate from the automatic decrement on order confirmation.

### Smart automation notifications (extends the existing bell)
- New types: **new customer** (first order from a phone number), **low
  stock** (any product ≤5 units), **stale pending** (an order sitting in
  `pending` >12h — tune `STALE_PENDING_HOURS` in `api/notifications.js`),
  **confirmed but no parcel** (status is `confirmed` but no Steadfast
  consignment exists 3+ hours later).

### Advanced Dashboard metrics
- Confirmation rate, delivery success rate, cancellation rate, return
  rate, Average Order Value, today's profit (Super Admin / Order Manager
  only — needs `viewFinance` permission), best-performing tenant, best
  category — all respecting the existing date-range filter.

### Invoice / Receipt system
- New **ইনভয়েস প্রিন্ট করুন** button on Order Details — a full A4 invoice
  (`src/utils/invoice.js`), separate from the small shipping label:
  itemized table, subtotal/discount/advance/total breakdown, a stable
  auto-generated invoice number (`INV-YYYYMMDD-XXXXXX`, derived from the
  order so reprints always show the same number), tenant branding (logo +
  name) if the order has a tenant, and a QR code.

### Tenant branding
- Tenants can now have a logo (Settings → Tenant management) — used on
  invoices automatically when that tenant is selected; falls back to the
  global brand name/logo from Settings otherwise.

### Reports Center (`/reports`)
- One page, 9 tabs: Sales, Profit (permission-gated), Products, Customers,
  Courier, Returns, Admin Performance, Inventory, Tenant — each with the
  same date-range filter as Dashboard and its own CSV export.
- Profit report matches sale price against cost price for items that
  carry a `productId` (i.e. added via the Create Order product picker);
  items without a known cost contribute ৳0 profit rather than a guess —
  noted directly on the report.

### Global search
- Search icon in the top bar (or open via clicking it — not yet wired to
  a keyboard shortcut) — searches Order ID, customer name, phone,
  tracking code, and product name/SKU/barcode in one box from any page,
  with results linking straight to the order or the Products page.

### Order workflow audit trail
- Status changes already went through `recordEdit`/the Audit Log; no
  structural change needed here beyond what v3.2 already logs (admin,
  action, timestamp, order) — this satisfies the "who/when/before→after"
  requirement via the existing Audit Log page.

## Explicitly not built (per user's instruction)
- **Customer Communication (SMS/WhatsApp automated messages)** — declined
  earlier in this project due to per-SMS cost; not built.
- **Full security hardening (JWT auth, refresh tokens, rate limiting,
  session/device management, backend RBAC middleware)** — user asked to
  skip this for now. The permission system above is enforced in the
  **frontend UI only** — the backend's `/api/orders` routes still have no
  authentication or server-side permission checks (documented in the
  backend README's "Notes on what's intentionally NOT here" section).
  This means a technically-savvy person hitting the API directly could
  bypass every permission gate described above. Worth revisiting before
  this handles real customer/payment data at scale.

## Also simplified / partial (being upfront about it)
- **Variant stock (color/size)** — not built; would need restructuring
  the product model into product+variant records, out of scope for this
  batch.
- **Barcode scanning** — barcode *generation* and *text search* by
  barcode value both work (Products page, Global Search); an actual
  scanner-device integration (most USB/Bluetooth barcode scanners just
  type into whatever's focused, so this may already work with a real
  scanner in the product search box — untested here) is not separately
  built.
- **Today's profit / Profit report** only account for items with a
  `productId` — orders from any other source show ৳0 cost for those
  lines rather than an invented number.
