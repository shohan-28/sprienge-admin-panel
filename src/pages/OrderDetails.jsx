...
/**
 * Safe date formatter.
 * Prevents Invalid Date from breaking the page.
 */
const formatDate = (value) => {
  if (!value) return "তারিখ পাওয়া যায়নি";
...
const formatTime = (value) => {
  if (!value) return "সময় পাওয়া যায়নি";
...
/**
 * getInvoiceNumber() যদি invoice.js-এর ভিতরে
 * invalid date পেয়ে error দেয়, তাহলে OrderDetails
 * page আর crash করবে না।
 */
const safeInvoiceNumber = (order) => {
  if (!order) return "N/A";

  try {
    return getInvoiceNumber(order) || "N/A";
  } catch (error) {
    console.error("Invoice Number Error:", error);

    const orderId = String(order?._id || "ORDER")
      .slice(-8)
      .toUpperCase();
...
const OrderDetails = () => {
  ...
  const canApproveReturns = hasPermission(admin, "approveReturns");
  ...
  const load = () => {
    ...
    getOrder(id)
      ...
      .catch((error) => {
        ...
        setOrder(null);
        setErr("অর্ডারটি খুঁজে পাওয়া যায়নি।");
      })
      .finally(() => {
        setLoading(false);
      });
  ...
  const isAssignedToMe =
    admin && assignment?.adminId === admin.id;
  ...
  const handleUnassign = () => {
    if (
      !confirm(
        "এই অর্ডার থেকে অ্যাসাইনমেন্ট সরিয়ে ফেলতে চান?"
      )
    ) {
      return;
    }
  ...
  const handleDelete = async () => {
    if (
      !confirm(
        "এই অর্ডারটি স্থায়ীভাবে মুছে ফেলতে চান?"
      )
    ) {
      return;
    }
  ...
  const handleSaveReturn = async () => {
    ...
    try {
      const updated = await updateOrder(id, {
        ...
        refundAmount:
          Number(returnForm.refundAmount) || 0,
  ...
  const itemsSubtotal = Array.isArray(form.items)
    ? form.items.reduce(
        (sum, it) =>
          sum +
          (Number(it.price) || 0) *
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      // Prevent double stock decrement.
  ...
  const handleCreateParcel = async (
    ...
    try {
      const updated =
        await createSteadfastParcel(id, {
          force,
        });
  ...
  const handlePrintLabel = async () => {
    ...
    try {
      ...
      alert(
        "Shipping label print করা যায়নি।"
      );
    } finally {
      setPrinting(false);
    }
  ...
  const handlePrintInvoice = () => {
    ...
    try {
      ...
      alert(
        "ইনভয়েস প্রিন্ট করা যায়নি। Invoice data বা date format যাচাই করুন।"
      );
    }
  };
        ...
        order.items.map((item, i) => (
          ...
          >
            ...
            {item.productImage ? (
              ...
              <div className="flex h-16 w-16 flex-shrink-0 items-center justify-center rounded-xl bg-mist-100 text-xs text-slate-300">
                No Image
              </div>
            )}
      ...
      {form.items.map((item, i) => (
        ...
        >
          ...
          {item.productImage ? (
            ...
            <div className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-xl bg-mist-100 text-[10px] text-slate-300">
              No Image
            </div>
          )}
