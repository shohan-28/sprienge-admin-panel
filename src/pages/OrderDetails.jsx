...
const OrderDetails = () => {
  ...
  const handlePrintInvoice = () => {
    const settings = getLabelSettings();
    const tenant = order.tenantId ? getTenantById(order.tenantId) : null;
    printInvoice(order, {
      brandName: tenant?.name || settings.brandName,
      brandLogoUrl: tenant?.logo || settings.brandLogoUrl,
      tenantName: tenant?.name || "",
    });
    if (admin) recordEdit(id, admin.id, "printed invoice", getInvoiceNumber(order));
  };
          ...
          <div className="space-y-5">
            <div className="no-print animate-in rounded-2xl border border-mist-200 bg-white p-6 shadow-card">
              <h3 className="mb-3 flex items-center gap-2 font-display text-base font-bold text-ink-900">
                অর্ডার স্ট্যাটাস
                {order.courierStatus && !admin?.canManageCourier && (
              ...
              {order.courierStatus && !admin?.canManageCourier && (
                <p className="mb-3 flex items-start gap-1.5 rounded-lg bg-mist-50 px-3 py-2 text-xs font-medium text-slate-500">
                  ...
                  Steadfast থেকে status নির্ধারিত হয়ে গেছে — শুধুমাত্র কুরিয়ার
                  ম্যানেজার এটা পরিবর্তন করতে পারবেন।
                </p>
              )}
              <div className="space-y-1.5">
                {STATUS_OPTIONS.map((s) => {
                  const locked = order.courierStatus && !admin?.canManageCourier;
                  return (
                    <button
                      key={s}
                      disabled={saving || locked}
                      onClick={() => handleStatusChange(s)}
                      title={locked ? "শুধুমাত্র কুরিয়ার ম্যানেজার পরিবর্তন করতে পারবেন" : ""}
                ...
                <div className="space-y-3">
                  ...
                  {canApproveReturns ? (
                    ...
                    <p className="text-xs font-medium text-rose-400">
                      শুধুমাত্র Super Admin বা Order Manager রিটার্ন/রিফান্ড
                      অনুমোদন করতে পারবেন।
                    </p>
                  )}
                </div>
            ...
            >
              <FileText size={15} />
              ইনভয়েস প্রিন্ট করুন ({getInvoiceNumber(order)})
            </button>
